...
export class PostService {
  ...
  async getUserLikedPosts(userId: string, page: number = 1, limit: number = 10) {
    ...
    const [likes, total] = await this.likeRepository.findAndCount({
      ...
      relations: ['post', 'post.category', 'post.category.parent', 'post.user'],
      skip,
      take: limit,
      order: { id: 'DESC' },
    });
    ...
    const enrichedPosts = await Promise.all(
      likes.map(async (like) => {
        ...
        const categoryHierarchy = post.category 
          ? await this.getCategoryHierarchy(post.category.id)
          : null;
