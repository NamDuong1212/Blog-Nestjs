...
export class ItineraryController {
  ...
  @ApiBearerAuth('token')
  @ApiOperation({
    summary: 'Delete an image from a specific day in the itinerary',
    description: 'Allows a user to delete an image from a specific day in their itinerary.',
  })
  @ApiResponse({
    status: 200,
  })
  @Delete('/:id/days/:dayNumber/image')
  @UseGuards(JwtAuthGuard)
  async deleteDayImage(
    ...
    try {
      const updatedDay = await this.itineraryService.deleteDayImage(
        id,
        dayNumber,
        req.user.id,
      );
